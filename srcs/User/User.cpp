...
void ISON(irc::Command *commnand);
...
void post_registration(irc::Command *command)
{
    ...
    command->reply(4, command->getServer().getConfig().get("name"),
        command->getServer().getConfig().get("version"),
        command->getServer().getConfig().get("user_mode"),
        command->getServer().getConfig().get("channel_givemode") + 
        command->getServer().getConfig().get("channel_togglemode") + 
        command->getServer().getConfig().get("channel_setmode"));
...
void irc::User::dispatch()
{
    ...
    for (std::vector<Command *>::iterator it = commands.begin(); it != commands.end(); ++it)
    {
        ...
        if (command_func.count((*it)->getPrefix()))
            command_func[(*it)->getPrefix()](*it);
        else if (DEBUG)
        {
            time_t t = time(0);
...
irc::User::User(int fd, struct sockaddr_t address): command_function(),
    fd(fd),
    buffer(),
    commands(),
    waitingToSend(),
    ...
    awayMessage()
{
    ...
    hoastaddr = inet_ntoa(address.sin_addr);
...
void irc::User::setDeleteMessage(std::string message) { this->deleteMessage = message; }
void irc::User::setAwayMessage(std::string message) { this->awayMessage = message; }
