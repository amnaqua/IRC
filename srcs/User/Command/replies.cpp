...
std::string RPL_WELCOME(std::string prefix) { return (":Welcome to the Internet Relay Network"); }
...
std::string RPL_TRACECONNECTION(std::string class1, std::string server) { return ("Try. " + class1 + " " + server); }
...
{
    return ("Serv " + class1 + " " + int1 + "S " + int2 + "C " + server + " " + nick + "!" + user + "@ " + host);
}
std::string RPL_TRACENEWTYPE(std::string type, std::string name) { return (type + "0 " + name); }
std::string RPL_STATSLINKINFO(std::string name, std::string q, std::string smessage, std::string sbytes, std::string rsmessage, std::string rbytes, std::string time)
{
    return (name + " " + q + " " + smessage + " " + sbytes + " " + rsmessage + " " + rbytes + " " + time);
}
...
std::string RPL_NOWAWAY() { return (":You have been merked as being away"); }
...
{
    return (mask + " " + server + " :" + hopcount + " " + info);
}
...
std::string ERR_NOSUCHNICK(std::string nickname) { return (nickname + " :No such nich/channel"); }
...
std::string ERR_TOOMANYCHANNELS(std::string channel) { return (channel + " :You have joined to many channels"); }
...
std::string ERR_NOADMININFO(std::string server) { return (server + " :No administrative info avaliable"); }
...
std::string ERR_ERRONEUSNICKNAME(std::string nick) { return (" :Erroneus nickname"); }
...
std::string ERR_NOTONCHANNEL(std::string channel) { return (channel + " :You're not on channel"); }
...
std::string ERR_NOTREGISTERED() { return (":You have not registred"); }
...
std::string irc::Command::getReplies(unsigned short code, std::string arg1, std::string arg2, std::string arg3, std::string arg4, std::string arg5, std::string arg6, std::string arg7)
{
    ...
    case 201:
        return (target + RPL_TRACECONNECTION(arg1, arg2));
    case 202:
